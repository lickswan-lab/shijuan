...
### v1.3.1 · 2026-04-22（当前）

**注释栏优化 / 阅读位置记录 / 大文件加载优化**

- AI 任务绑定到注释（不再"切走后仍显示正在思考"）：每个 AI 回答是注释里的一条 entry，状态图标 running / completed / failed / aborted；运行中切别的注释/文献，AI 继续跑，回来能看到完整结果
- 注释列表按页码 / 章节 / 段落分组，可折叠展开；当前阅读位置对应的组默认展开
- 其他文献的注释 三层嵌套（文献 → 页/章 → 注释），默认全折叠，按需展开
- 重开文献自动恢复到上次阅读位置（PDF / OCR / EPUB / DOCX / HTML / TXT / MD 全格式支持）；EPUB 用 CFI、其他用 scrollTop；OCR / PDF 视图位置分开存
- 大 PDF 懒加载（IntersectionObserver）+ scale 防抖，500 页 PDF 不再卡死
- EPUB 多项修复：阅读进度条工作（locations 索引）、向上翻不再闪回（overflow-anchor）、注释栏开关后自动 resize
- AI 状态图标全面升级：暖色 SVG + 圆角 pill，不再是裸 unicode 符号
- AI 响应 idle timeout 180s（取代 60s hard cap），long task 不被截断
- max_tokens 提到 16384（修了 GLM 等 provider 默认 1024 截断长答复）
- 注释里"问：XXX"字号加大；编辑框透明化与原注释块融合
- 各种小修：注释面板按钮防换行、"问：XXX"字号、padding 等
