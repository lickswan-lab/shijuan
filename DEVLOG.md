...
1. OCR 范围与续扫逻辑
   - OCR 弹窗会显示已识别页数与剩余页数。
   - 后续 OCR 会自动拼接到已有文本；如果新 OCR 页码覆盖旧结果，则以新结果替换覆盖区间。
   - 修正已 OCR 页面检测，避免整本识别后仍误判未完成。

2. 注释与 AI 生成块
   - 注释 AI 支持“重新生成”，用户可填写反馈后替换原块内容，块位置保持不变。
   - 生成中状态在刷新后不再卡死，会恢复为可继续操作的中断态。
   - Markdown 表格在注释栏中支持横向滚动和表格样式。
   - 召唤人物批注拥有独立 AI 区块样式，不再融入背景。
   - 生成中“停止”按钮改为更轻量的胶囊样式。

3. 阅读器与 UI
   - 顶部 OCR / PDF 工具栏压缩高度并统一按钮质感。
   - 跳转页码输入与按钮对齐，视觉更稳。
   - 深色模式下新建笔记命名输入的对比度增强。
   - 注释列表不再显示 AI 状态标记，减少视觉噪音。

4. Skill 与模型接入
   - 新增涂尔干 skill。
   - API Key 接入列表按官方文档校正部分厂商模型名与参数字段。
   - 明确不新增未确认或不需要维护的 Qwen3 新模型入口。

5. 官网
   - 官网版本、下载链接、meta 描述与 hero 文案同步到 v1.3.6。
