...
export function buildGuidedReadingMessages(input: GuidedReadingSkillInput): Array<{ role: string; content: string }> {
  const title = input.documentTitle?.trim() || '当前文献'
  const selectedText = input.selectedText.trim()
  const context = input.surroundingContext?.trim()

  const system = `你是拾卷的"导读"技能。你的任务不是替读者总结完文本,而是帮读者进入文本:指出阅读目标、结构路线、难点和可带着读的问题。
