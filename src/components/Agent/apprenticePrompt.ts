...
(一段不超过 3 句的"整体感"——这段时间的思考像什么?如果没什么可说,只写 1 句)
...
> (1 个开放、具体的问题。不是"你觉得怎么样",而是指向这段时间某个具体点。)

---
*数据:N 天活跃,打开 X 部文献,写下 Y 条思考。*
\`\`\`

（标题里的{日期范围}请按用户数据填写真实范围,比如"最近 7 天"、"2026-04-01 ~ 2026-04-15"或"第 16 周"——match 用户选的跨度）
...
${entriesText || '  (仅创建了注释,未写内容)'}`
  ...
  const prevText = ctx.previousWeekLog
    ? `\n\n---\n\n**上一次的观察(供参考,避免重复;但如果模式延续可以深化):**\n\n${ctx.previousWeekLog.slice(0, 2000)}`
    : ''

  // Derive a human-friendly range label from weekStart/weekEnd.
  // If weekCode is an ISO-week code (YYYY-Wxx), render as "第 xx 周";
  // else render as "YYYY-MM-DD ~ YYYY-MM-DD".
  const startDate = new Date(ctx.weekStart).toISOString().slice(0, 10)
  const endDateExclusive = new Date(ctx.weekEnd).getTime() - 1
  const endDate = new Date(endDateExclusive).toISOString().slice(0, 10)
  const spanDays = Math.max(1, Math.round((new Date(ctx.weekEnd).getTime() - new Date(ctx.weekStart).getTime()) / 86400000))
  const isIsoWeek = /^\d{4}-W\d{2}$/.test(ctx.weekCode)
  const rangeLabel = isIsoWeek
    ? `第 ${ctx.weekCode.split('-W')[1]} 周（${startDate} ~ ${endDate}）`
    : spanDays === 7
      ? `最近 7 天（${startDate} ~ ${endDate}）`
      : `${startDate} ~ ${endDate}（${spanDays} 天）`

  const user = `## 这是 ${rangeLabel} 的痕迹
...
现在,根据痕迹写一份 ${rangeLabel} 的观察报告。报告标题里的日期范围要和这段时间 match。记住:不是总结,是观察。说出他自己没注意到的模式。如果数据不够,诚实地说。`
